...
CREATE DATABASE [ IF NOT EXISTS] 数据库名 数据库选项
	-- 数据库选项：
	CHARACTER SET charset_name -- 设置数据库字符集
  COLLATE collation_name -- 设置列排序所用字符集
  
  -- 示例 创建fish数据库
  CREATE DATABASE IF NOT EXISTS `fish` DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
```
...
-- 修改表
    ...
    -- RENAME可以交换两个表名
        eg: rename table t1 to t3,t2 to t1,t2 to t3;
    -- 修改表的字段机构
    ALTER TABLE 表名 操作名
        -- 操作名
        ADD[ COLUMN] 字段定义 -- 增加字段
        AFTER 字段名          -- 表示增加在该字段名后面
            eg: ALTER TABLE test ADD COLUMN c INT NOT NULL AFTER a -- c列加在a后面
        FIRST                   -- 表示增加在第一个
